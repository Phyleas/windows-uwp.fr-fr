...
|--------------|---------|
| Rendez-vous, contacts et messages électroniques | Toutes les API ne sont actuellement pas prises en charge. |
| Authentification et identité des utilisateurs | Toutes les API ne sont actuellement pas prises en charge. |
| Tâches en arrière-plan | Les applications de bureau peuvent inscrire des tâches d'arrière-plan, mais seules les applications entièrement UWP peuvent s'exécuter <i>en tant que</i> tâches d'arrière-plan. Pour plus d’informations, voir [l’exemple de tâche en arrière-plan sur GitHub](https://github.com/Microsoft/DesktopBridgeToUWP-Samples/tree/master/Samples/BackgroundTasksSample). |
| Bluetooth | Toutes les API ne sont actuellement pas prises en charge. |
| Chat et appels téléphoniques | Les applications de bureau ne peuvent pas s'exécuter sur un appareil mobile tant qu'elles n'ont pas été entièrement transférées vers UWP. |
| Activation des contrats | Seules les applications UWP complètes peuvent être activées par un contrat. |
| Chiffrement | Toutes les API ne sont actuellement pas prises en charge. |
| Géolocalisation | Toutes les API ne sont actuellement pas prises en charge. |
| Sélecteurs de fichiers et de dossiers | Les applications avec identité disposent d'un accès complet au système de fichiers et n'ont pas besoin de sélecteurs UWP. |
| Médias | Toutes les API ne sont actuellement pas prises en charge. |
| Point de service | Toutes les API ne sont actuellement pas prises en charge. |
| Cartes à puce | Toutes les API ne sont actuellement pas prises en charge. |
| PlayReady | Toutes les API ne sont actuellement pas prises en charge. |
| Réseau privé virtuel | Toutes les API ne sont actuellement pas prises en charge. |
| Portefeuille | Les applications de bureau ne peuvent pas s'exécuter sur un appareil mobile tant qu'elles n'ont pas été entièrement transférées vers UWP. |
| Interface utilisateur XAML | La version 1809 et les versions antérieures de Windows 10 ne prennent pas en charge l'hébergement de contenu XAML UWP dans les applications de bureau. À partir de Windows 10, version 1903, vous pouvez utiliser la fonctionnalité [XAML Islands](xaml-islands.md) pour héberger du contenu XAML UWP.  |
...
Les API dotées de l'attribut [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute) sont prises en charge dans toutes les applications de bureau, qu'elles disposent ou non de l'identité du package. Pour déterminer si une API que vous souhaitez appeler à partir de votre application de bureau est prise en charge, recherchez l'article de référence correspondant à la classe ou au membre qui vous intéresse dans la [documentation de référence des espaces de noms WinRT](/uwp/api/). Vérifiez ensuite si la section **Attributs** de l'article de référence inclut l'attribut [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute).
