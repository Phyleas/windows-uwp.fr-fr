...
La classe [**StorageLibraryChangeTracker**](/uwp/api/Windows.Storage.StorageLibraryChangeTracker) permet aux applications de suivre les modifications apportées aux fichiers et aux dossiers au fur et à mesure que les utilisateurs les déplacent dans le système. À l'aide de la classe **StorageLibraryChangeTracker**, une application peut suivre :
...
- Assurez-vous que votre application dispose de l'autorisation d'accès à la bibliothèque appropriée dans le manifeste avant de créer l'objet [**StorageLibrary**](/uwp/api/windows.storage.storagelibrary). Pour plus d'informations, consultez [Autorisations d'accès aux fichiers](./file-access-permissions.md).
- La méthode [**Enable**](/uwp/api/windows.storage.storagelibrarychangetracker.enable) est thread-safe, elle ne réinitialise pas votre pointeur et elle peut être appelée autant de fois que vous le souhaitez (nous y reviendrons ultérieurement).
...
Une fois initialisé, le traceur de modifications commence à enregistrer toutes les opérations effectuées dans une bibliothèque, même lorsque l'application n'est pas en cours d'exécution. Les applications peuvent s'inscrire à l'événement [**StorageLibraryChangedTrigger**](/uwp/api/Windows.ApplicationModel.Background.StorageLibraryContentChangedTrigger) pour déclencher une activation chaque fois qu'une modification intervient.
...
Au terme du traitement des modifications, l'application doit indiquer au système de ne plus jamais afficher ces modifications en appelant la méthode [**AcceptChangesAsync**](/uwp/api/windows.storage.storagelibrarychangereader.acceptchangesasync).
...
Dans ce cas, **ReadBatchAsync** renvoie le code d'erreur [**StorageLibraryChangeType.ChangeTrackingLost**](/uwp/api/windows.storage.storagelibrarychangetype). Si votre application reçoit ce code d'erreur, cela signifie deux choses :
...
* Le traceur de modifications ne renverra plus de modifications tant que vous n'aurez pas appelé [**Reset**](/uwp/api/windows.storage.storagelibrarychangetracker.reset). Une fois Reset appelé par l'application, le pointeur sera déplacé vers la modification la plus récente et le suivi reprendra normalement.
...
La classe [**StorageLibrary**](/uwp/api/windows.storage.storagelibrary) est un groupe virtuel de dossiers racine contenant d'autres dossiers. Pour la rapprocher d'un traceur de modifications de système de fichiers, nous avons fait les choix suivants :

- Toute modification apportée aux descendants des dossiers de la bibliothèque racine sera représentée dans le traceur de modifications. Les dossiers de la bibliothèque racine sont accessibles à l'aide de la propriété [**Folders**](/uwp/api/windows.storage.storagelibrary.folders).
- L'ajout ou la suppression de dossiers racine à partir d'un objet **StorageLibrary** (via [**RequestAddFolderAsync**](/uwp/api/windows.storage.storagelibrary.requestaddfolderasync) et [**RequestRemoveFolderAsync**](/uwp/api/windows.storage.storagelibrary.requestremovefolderasync)) ne crée aucune entrée dans le traceur de modifications. Ces modifications peuvent être suivies via l'événement [**DefinitionChanged**](/uwp/api/windows.storage.storagelibrary.definitionchanged) ou en énumérant les dossiers racine de la bibliothèque à l'aide de la propriété [**Folders**](/uwp/api/windows.storage.storagelibrary.folders).
...
Les applications doivent appeler [**Enable**](/uwp/api/windows.storage.storagelibrarychangetracker.enable) dès qu'elles entament le suivi du système de fichiers et avant chaque énumération des modifications. Toutes les modifications pourront ainsi être capturées par le traceur de modifications.  
