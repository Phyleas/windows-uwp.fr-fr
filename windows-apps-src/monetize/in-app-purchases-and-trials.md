...
|---------|-------------------|
| SKU |  L’ID Store d'une référence (SKU) présente le format ```<product Store ID>/xxxx```, où ```xxxx``` est une chaîne alphanumérique de 4 caractères qui identifie une référence (SKU) du produit. Par exemple, ```9NBLGGH4R315/000N```. Cet ID est retourné par la propriété [StoreID](https://docs.microsoft.com/uwp/api/windows.services.store.storesku.storeid) d’un objet [StoreSku](https://docs.microsoft.com/uwp/api/windows.services.store.storesku) et il est parfois appelé *ID Windows Store de référence*. |
