...
Un ensemble de 6 éléments [**Ellipse**](/uwp/api/Windows.UI.Xaml.Shapes.Ellipse) font partie du modèle du contrôle [**ProgressRing**](/uwp/api/Windows.UI.Xaml.Controls.ProgressRing), et 2 éléments **Ellipse** concentriques font partie d’une classe [**RadioButton**](/uwp/api/Windows.UI.Xaml.Controls.RadioButton).
...
Une forme [**Rectangle**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle) a quatre côtés dont les côtés opposés sont de mesure égale. Pour créer une forme **Rectangle** basique, spécifiez une largeur [**Width**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Width), une hauteur [**Height**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Height) et un remplissage [**Fill**](/uwp/api/windows.ui.xaml.shapes.shape.fill).

Vous pouvez arrondir les angles d’une forme [**Rectangle**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle). Pour ce faire, indiquez une valeur pour les propriétés [**RadiusX**](/uwp/api/windows.ui.xaml.shapes.rectangle.radiusx) et [**RadiusY**](/uwp/api/windows.ui.xaml.shapes.rectangle.radiusy). Ces propriétés spécifient les axes x et y d’une ellipse définissant la courbe des angles. La valeur maximale de **RadiusX** correspond à la valeur [**Width**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Width) divisée par deux, et la valeur maximale de **RadiusY** à celle de [**Height**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Height) divisée par deux.

L’exemple suivant permet de créer une forme [**Rectangle**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle) de largeur [**Width**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Width) égale à 200 et de hauteur [**Height**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Height) égale à 100. La valeur [**Blue**](/uwp/api/windows.ui.colors.blue) est utilisée pour le pinceau [**SolidColorBrush**](/uwp/api/Windows.UI.Xaml.Media.SolidColorBrush) comme propriété [**Fill**](/uwp/api/windows.ui.xaml.shapes.shape.fill), et la valeur [**Black**](/uwp/api/windows.ui.colors.black) est utilisée pour le pinceau **SolidColorBrush** comme propriété [**Stroke**](/uwp/api/windows.ui.xaml.shapes.shape.stroke). Nous avons défini la propriété [**StrokeThickness**](/uwp/api/windows.ui.xaml.shapes.shape.strokethickness) sur 3. Nous avons défini la propriété [**RadiusX**](/uwp/api/windows.ui.xaml.shapes.rectangle.radiusx) sur 50 et la propriété [**RadiusY**](/uwp/api/windows.ui.xaml.shapes.rectangle.radiusy) sur 10 afin d’obtenir une forme **Rectangle** aux angles arrondis.
...
**Conseil**  Une valeur [**Point**](/uwp/api/Windows.Foundation.Point) est souvent utilisée en tant que type en XAML pour les situations autres que la déclaration des sommets des formes. Par exemple, un objet **Point** fait partie des données d’événement pour les événements tactiles, si bien que vous pouvez déterminer exactement l’endroit auquel s’est produit une action tactile dans un espace de coordonnées. Pour plus d’informations sur **Point** et sur la façon de l’utiliser en XAML ou dans du code, voir la rubrique de référence d’API pour [**Point**](/uwp/api/Windows.Foundation.Point).

## <a name="line"></a>Ligne

Un objet [**Line**](/uwp/api/Windows.UI.Xaml.Shapes.Line) représente simplement une ligne tracée entre deux points dans un espace de coordonnées. Un objet **Line** ignore toute valeur fournie pour [**Fill**](/uwp/api/windows.ui.xaml.shapes.shape.fill), car il est dépourvu d’espace intérieur. Dans le cas d’un objet **Line**, veillez à spécifier des valeurs pour les propriétés [**Stroke**](/uwp/api/windows.ui.xaml.shapes.shape.stroke) et [**StrokeThickness**](/uwp/api/windows.ui.xaml.shapes.shape.strokethickness), sinon l’objet **Line** ne sera pas restitué.

Vous n’utilisez pas de valeurs [**Point**](/uwp/api/Windows.Foundation.Point) pour spécifier une forme [**Line**](/uwp/api/Windows.UI.Xaml.Shapes.Line) ; à la place, vous utilisez des valeurs [**Double**](/dotnet/api/system.double) discrète pour [**X1**](/uwp/api/windows.ui.xaml.shapes.line.x1), [**Y1**](/uwp/api/windows.ui.xaml.shapes.line.y1), [**X2**](/uwp/api/windows.ui.xaml.shapes.line.x2) et [**Y2**](/uwp/api/windows.ui.xaml.shapes.line.y2). Cela permet un balisage minimal pour les lignes horizontales ou verticales. Par exemple, `<Line Stroke="Red" X2="400"/>` définit une ligne horizontale de 400 pixels de long. Les autres propriétés X,Y étant par défaut définies sur 0, ce code XAML trace une ligne entre les points `(0,0)` et `(400,0)`. Vous pouvez ensuite utiliser une classe [**TranslateTransform**](/uwp/api/Windows.UI.Xaml.Media.TranslateTransform) pour déplacer toute la classe **Line**, si vous voulez qu’elle commence à un point autre que (0,0).
...
**Remarque**   Vous pouvez paramétrer explicitement des points de départ et de fin identiques dans la propriété [**Points**](/uwp/api/windows.ui.xaml.shapes.polyline.points) définie pour l’objet [**Polyline**](/uwp/api/Windows.UI.Xaml.Shapes.Polyline), mais dans ce cas l’utilisation d’un objet [**Polygon**](/uwp/api/Windows.UI.Xaml.Shapes.Polygon) est probablement mieux appropriée.

Si vous spécifiez une propriété [**Fill**](/uwp/api/windows.ui.xaml.shapes.shape.fill) d’un objet [**Polyline**](/uwp/api/Windows.UI.Xaml.Shapes.Polyline), la propriété **Fill** peint l’espace intérieur de la forme, même si les points de départ et de fin de la propriété [**Points**](/uwp/api/windows.ui.xaml.shapes.polyline.points) définie pour l’objet **Polyline** ne se recoupent pas. Si vous ne spécifiez pas de propriété **Fill**, l’objet **Polyline** est similaire à celui qui serait restitué si vous indiquiez plusieurs éléments [**Line**](/uwp/api/Windows.UI.Xaml.Shapes.Line) avec intersection des points de départ et de fin de lignes consécutives.

Comme dans le cas d’une forme [**Polygon**](/uwp/api/Windows.UI.Xaml.Shapes.Polygon), la propriété [**Points**](/uwp/api/windows.ui.xaml.shapes.polyline.points) définit l’ensemble des points constituant la limite. En langage XAML, vous devez définir les points à l’aide d’une liste séparée par des virgules. En code-behind, vous devez utiliser un élément [**PointCollection**](/uwp/api/Windows.UI.Xaml.Media.PointCollection) pour définir les points et ajouter chaque point à l’ensemble sous la forme d’une structure [**Point**](/uwp/api/Windows.Foundation.Point).

Cet exemple crée une forme [**Polyline**](/uwp/api/Windows.UI.Xaml.Shapes.Polyline) à quatre points définis sur `(10,200)`, `(60,140)`, `(130,140)` et `(180,200)`. Une propriété [**Stroke**](/uwp/api/windows.ui.xaml.shapes.shape.stroke) est définie, contrairement à une propriété [**Fill**](/uwp/api/windows.ui.xaml.shapes.shape.fill).
...
Voici le rendu de la forme [**Polyline**](/uwp/api/Windows.UI.Xaml.Shapes.Polyline). Remarquez que les premier et dernier points ne sont pas reliés par le contour [**Stroke**](/uwp/api/windows.ui.xaml.shapes.shape.stroke) comme ils le sont dans le cas d’une forme [**Polygon**](/uwp/api/Windows.UI.Xaml.Shapes.Polygon).
...
Pour spécifier la géométrie d’un tracé, utilisez la propriété [**Data**](/uwp/api/windows.ui.xaml.shapes.path.data). Deux techniques permettent de définir la propriété **Data** :

- Vous pouvez définir une valeur de chaîne pour [**Data**](/uwp/api/windows.ui.xaml.shapes.path.data) en XAML. Ainsi, la valeur **Path.Data** utilise un format de sérialisation pour les graphismes. En règle générale, vous ne modifiez pas le texte de cette valeur sous la forme d’une chaîne une fois qu’elle a été établie. À la place, vous utilisez des outils de conception qui vous permettent de travailler dans une métaphore de conception ou de dessin sur une surface. Ensuite, vous enregistrez ou exportez la sortie, ce qui vous permet d’obtenir un fichier XAML ou un fragment de chaîne XAML comportant les informations de la propriété **Path.Data**.
- Vous pouvez définir la propriété [**Data**](/uwp/api/windows.ui.xaml.shapes.path.data) sur un objet [**Geometry**](/uwp/api/Windows.UI.Xaml.Media.Geometry) unique. Cette opération peut être réalisée dans du code ou en XAML. Cet objet **Geometry** unique est généralement un objet [**GeometryGroup**](/uwp/api/windows.ui.xaml.media.geometrygroup), qui fait office de conteneur pouvant, à partir de plusieurs définitions de géométrie, composer un objet destiné au modèle objet. Cette approche permet notamment de définir une ou plusieurs des courbes et des formes complexes en tant que valeurs [**Segments**](/uwp/api/windows.ui.xaml.media.pathfigure.segments) d’un objet [**PathFigure**](/uwp/api/Windows.UI.Xaml.Media.PathFigure), par exemple [**BezierSegment**](/uwp/api/Windows.UI.Xaml.Media.BezierSegment).

L’exemple ci-dessous montre un objet [**Path**](/uwp/api/Windows.UI.Xaml.Shapes.Path) qui aurait pu être obtenu en utilisant Blend pour Visual Studio pour produire une série limitée de formes vectorielles, puis en enregistrant le résultat au format XAML. L’objet **Path** global se compose d’une courbe de Bézier et d’une ligne. L’exemple vise essentiellement à illustrer une partie des éléments du format de sérialisation [**Path.Data**](/uwp/api/windows.ui.xaml.shapes.path.data) et de ce que les nombres représentent.

Cette propriété [**Data**](/uwp/api/windows.ui.xaml.shapes.path.data) commence avec une commande de mouvement, indiquée par la mention « M », qui établit un point de départ absolu du tracé.

Le premier segment est une courbe de Bézier cubique commençant à `(100,200)` et se terminant à `(400,175)`. Elle est dessinée à l’aide des deux points de contrôle `(100,25)` et `(400,350)`. Ce segment est indiqué par la commande « C » dans la chaîne de l’attribut [**Data**](/uwp/api/windows.ui.xaml.shapes.path.data).
