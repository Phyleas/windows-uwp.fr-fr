...
| Terme | Description |
|------|-------------|
| Propriété de dépendance | Propriété qui existe sur un identificateur [**DependencyProperty**](/uwp/api/Windows.UI.Xaml.DependencyProperty) (voir ci-dessous). Cet identificateur est généralement disponible en tant que membre statique de la classe dérivée **DependencyObject** de définition. |
...
Pour plus d’informations, voir [Animations dans une table de montage séquentiel](../design/motion/storyboarded-animations.md).

### <a name="default-values"></a>Valeurs par défaut

L’établissement de la valeur par défaut pour une propriété de dépendance dont la valeur est [**PropertyMetadata**](/uwp/api/Windows.UI.Xaml.PropertyMetadata) est expliqué plus en détail dans la rubrique [Propriétés de dépendance personnalisées](custom-dependency-properties.md).
...
- Une propriété qui utilise un objet au moment de l’exécution ou le type **Object** (un *type de référence*) a une valeur par défaut égale à **null**. Par exemple, la propriété [**DataContext**](/uwp/api/windows.ui.xaml.frameworkelement.datacontext) a la valeur **null** jusqu’à ce qu’elle soit volontairement définie ou héritée.
...
- Une propriété qui utilise une chaîne ([**System.String**](/dotnet/api/system.string) pour .NET, [**Platform::String**](/cpp/cppcx/platform-string-class) pour C++/CX) a la valeur par défaut d’une chaîne vide (**""**).
...
Windows 10 introduit la méthode [**RegisterPropertyChangedCallback**](/uwp/api/windows.ui.xaml.dependencyobject.registerpropertychangedcallback). Cette méthode permet au code d’application de s’inscrire aux notifications de modification lorsque la propriété de dépendance spécifiée est modifiée sur une instance de [**DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject).
