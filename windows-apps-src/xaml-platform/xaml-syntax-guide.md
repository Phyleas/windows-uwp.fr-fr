...
De nombreuses propriétés de collection sont également identifiées en tant que propriété de contenu XAML pour la classe. Il est fréquent de voir la combinaison du traitement de collections implicites et de la syntaxe de contenu XAML dans des types utilisés pour la composition de contrôles, tels que des panneaux, des vues ou des contrôles d’éléments. Par exemple, l’exemple suivant montre le XAML le plus simple possible pour la composition de deux éléments d’interface utilisateur homologue dans un [**StackPanel**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.StackPanel).
...
**Remarque**   XAML concepteurs promeuvent souvent des conventions de classement si vous utilisez des surfaces de conception autres que l’éditeur XML, mais vous pouvez modifier librement ce XAML ultérieurement, pour réorganiser les attributs ou en introduire de nouveaux.
