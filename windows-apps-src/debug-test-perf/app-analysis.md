...
description: Découvrez l’outil App Analysis, ainsi que les instructions et bonnes pratiques relatives aux performances qu’il utilise pour évaluer votre code d’application.
ms.date: 02/08/2017
ms.topic: article
...
L’application utilise SetSource() au lieu de SetSourceAsync(). Évitez toujours d’utiliser [**SetSource**](/uwp/api/windows.ui.xaml.media.imaging.bitmapsource.setsource) et utilisez plutôt [**SetSourceAsync**](/uwp/api/windows.ui.xaml.media.imaging.bitmapsource.setsourceasync) quand vous définissez un flux pour décoder des images de manière asynchrone. 
...
Vous pouvez aussi définir une taille de décodage explicite pour créer une version de l’image à la taille exacte qui sera affichée à l’écran à l’aide des propriétés [**DecodePixelWidth**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelwidth) et [**DecodePixelHeight**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelheight).
...
Définissez une taille de décodage explicite pour créer une version de l’image à la taille exacte qui sera affichée à l’écran à l’aide des propriétés [**DecodePixelWidth**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelwidth) et [**DecodePixelHeight**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelheight).
...
Les images qui s’affichent très grandes à l’écran utilisent le décodage à la taille naturelle. Définissez une taille de décodage explicite pour créer une version de l’image à la taille exacte qui sera affichée à l’écran à l’aide des propriétés [**DecodePixelWidth**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelwidth) et [**DecodePixelHeight**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelheight).
...
Quand une image est utilisée pour un [**NineGrid**](/uwp/api/windows.ui.xaml.controls.image.ninegrid), elle utilise un chemin de rastérisation logicielle qui ne met pas du tout les images à l’échelle. En outre, elle doit stocker une copie de l’image dans la mémoire logicielle et matérielle. Quand vous utilisez **NineGrid**, votre application doit effectuer une mise à l’échelle préalable de ses images à leur taille de rendu approximative.
...
Pour obtenir la liste complète des bonnes pratiques qui permettent d’améliorer les performances de démarrage de votre application, voir [Meilleures pratiques en matière de performances de démarrage de votre application](./best-practices-for-your-app-s-startup-performance.md) et [Assurer la réactivité du thread de l’interface utilisateur](./keep-the-ui-thread-responsive.md).
