...
-   Animations sur les propriétés [**Canvas.Left**](/dotnet/api/system.windows.controls.canvas.left) et [**Canvas.Top**](/dotnet/api/system.windows.controls.canvas.top)
...
-   Animations sur les propriétés [**UIElement**](/uwp/api/Windows.UI.Xaml.UIElement) suivantes lors du ciblage de sous-propriétés des types de valeur de retour suivants :
...
Par exemple, si votre application utilise un objet [**RandomAccessStream**](/uwp/api/Windows.Storage.Streams.RandomAccessStream) ou [**IInputStream**](/uwp/api/Windows.Storage.Streams.IInputStream), veillez à appeler la méthode close sur l’objet lorsque votre application ne l’utilise plus, de manière à libérer l’objet sous-jacent.

### <a name="display-full-screen-video-playback-when-possible"></a>Afficher la lecture vidéo en plein écran dès que possible

Dans les applications UWP, utilisez toujours la propriété [**IsFullWindow**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement.isfullwindow) sur l’objet [**MediaPlayerElement**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement) pour activer et désactiver le rendu de fenêtre entière. L’utilisation des optimisations au niveau système est ainsi garantie pendant la lecture multimédia.
...
Pour empêcher que l’affichage ne soit désactivé lorsque plus aucune action utilisateur n’est détectée (par exemple quand une application lit une vidéo), vous pouvez appeler [**DisplayRequest.RequestActive**](/uwp/api/windows.system.display.displayrequest.requestactive).

Pour économiser l’énergie et prolonger la durée de vie de la batterie, vous devez appeler [**DisplayRequest.RequestRelease**](/uwp/api/windows.system.display.displayrequest.requestrelease) pour libérer la demande d’affichage dès qu’elle n’est plus nécessaire.
...
Les moteurs multimédias sont des objets coûteux et l’infrastructure XAML retarde le chargement des fichiers .dll et la création des objets volumineux le plus longtemps possible. L’objet [**MediaPlayerElement**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement) doit se charger de ce travail une fois que la source est définie via la propriété [**Source**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement.source). Lorsque ce réglage a lieu une fois que l’utilisateur est prêt à lire le média, la majorité du coût lié à l’objet **MediaPlayerElement** est retardée aussi longtemps que possible.

### <a name="set-mediaplayerelementpostersource"></a>Définir MediaPlayerElement.PosterSource

La définition de [**MediaPlayerElement.PosterSource**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement.postersource) permet au code XAML de libérer certaines ressources du GPU qui auraient été utilisées d’une autre façon. Cette API permet à une application d’utiliser aussi peu de mémoire que possible.
...
-   Mettez à jour la valeur de l’objet [**Slider**](/uwp/api/Windows.UI.Xaml.Controls.Slider) en fonction d’un minuteur qui interroge la propriété [**Position**](/uwp/api/windows.media.playback.mediaplaybacksession.position) sur l’élément [**MediaPlayerElement.MediaPlayer**](/uwp/api/windows.ui.xaml.controls.mediaplayerelement.mediaplayer). Veillez à utiliser une fréquence de mise à jour raisonnable pour votre minuteur. La propriété **Position** est seulement mise à jour toutes les 250 millisecondes lors de la lecture.
-   La taille de la fréquence d’incrémentation du l’objet Slider doit s’adapter à la longueur de la vidéo.
-   Abonnez-vous aux événements [**PointerPressed**](/uwp/api/windows.ui.xaml.uielement.pointerpressed), [**PointerMoved**](/uwp/api/windows.ui.xaml.uielement.pointermoved) et [**PointerReleased**](/uwp/api/windows.ui.xaml.uielement.pointerreleased) sur l’objet Slider pour affecter à la propriété [**PlaybackRate**](/uwp/api/windows.media.playback.mediaplaybacksession.playbackrate) la valeur 0 lorsque l’utilisateur fait glisser le curseur.
-   Dans le gestionnaire d’événements [**PointerReleased**](/uwp/api/windows.ui.xaml.uielement.pointerreleased), définissez manuellement la position du média sur la valeur de la position du curseur afin d’obtenir un alignement du curseur optimal lors du nettoyage.
...
-   La [**BitmapImage**](/uwp/api/Windows.UI.Xaml.Media.Imaging.BitmapImage) est associée à l’arborescence XAML dynamique après la définition du contenu avec [**SetSourceAsync**](/uwp/api/windows.ui.xaml.media.imaging.bitmapsource.setsourceasync) ou [**UriSource**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.urisource).
-   L’image est décodée à l’aide d’un décodage synchrone comme [**SetSource**](/uwp/api/windows.ui.xaml.media.imaging.bitmapsource.setsource).
-   L’image est masquée en définissant [**Opacity**](/uwp/api/Windows.UI.Xaml.UIElement.Opacity) sur 0 ou [**Visibility**](/uwp/api/windows.ui.xaml.uielement.visibility) sur **Collapsed** sur l’élément d’image hôte, le pinceau, ou tout autre élément parent.
-   Le contrôle d’image ou le pinceau utilise un [**Stretch**](/uwp/api/Windows.UI.Xaml.Media.Stretch) de **None**.
-   L’image est utilisée en tant que [**NineGrid**](/uwp/api/windows.ui.xaml.controls.image.ninegrid).
...
Par conséquent, vous devez utiliser [**UriSource**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.urisource) pour l’intégration lors du chargement d’images à partir d’un package d’application et éviter d’utiliser un flux de fichier et [**SetSourceAsync**](/uwp/api/windows.ui.xaml.media.imaging.bitmapsource.setsourceasync) lorsque ce n’est pas obligatoire.
...
La classe [**SoftwareBitmapSource**](/uwp/api/Windows.UI.Xaml.Media.Imaging.SoftwareBitmapSource) échange des images non compressées interopérables entre les différents espaces de noms WinRT, tels que [**BitmapDecoder**](/uwp/api/Windows.Graphics.Imaging.BitmapDecoder), les API de caméra et XAML. Cette classe évite une copie supplémentaire qui d’ordinaire serait nécessaire avec [**WriteableBitmap**](/uwp/api/Windows.UI.Xaml.Media.Imaging.WriteableBitmap) et qui aide à réduire la mémoire maximale et la latence de la source à l’écran.

Le [**SoftwareBitmap**](/uwp/api/Windows.Graphics.Imaging.SoftwareBitmap) qui fournit les informations sources peut également être configuré pour utiliser un [**IWICBitmap**](/windows/desktop/api/wincodec/nn-wincodec-iwicbitmap) personnalisé pour fournir un magasin de stockage rechargeable qui permet à l’application de remapper la mémoire comme il convient. Il s’agit d’un exemple d’utilisation avancée de C++.

Votre application doit utiliser [**SoftwareBitmap**](/uwp/api/Windows.Graphics.Imaging.SoftwareBitmap) et [**SoftwareBitmapSource**](/uwp/api/Windows.UI.Xaml.Media.Imaging.SoftwareBitmapSource) pour interopérer avec d’autres API WinRT qui produisent et utilisent des images. Et votre application doit utiliser **SoftwareBitmapSource** lors du chargement de données d’image non compressées au lieu d’utiliser [**WriteableBitmap**](/uwp/api/Windows.UI.Xaml.Media.Imaging.WriteableBitmap).

### <a name="use-getthumbnailasync-for-thumbnails"></a>Utiliser GetThumbnailAsync pour les miniatures

Un exemple d’utilisation des images mises à l’échelle : la création des miniatures. Bien que vous puissiez utiliser [**DecodePixelWidth**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelwidth) et [**DecodePixelHeight**](/uwp/api/windows.ui.xaml.media.imaging.bitmapimage.decodepixelheight) pour fournir des petites versions des images, UWP fournit des API plus efficaces pour récupérer les miniatures. [**GetThumbnailAsync**](/uwp/api/windows.storage.storagefile.getthumbnailasync) fournit les miniatures pour les images dont le système de fichiers est déjà mis en cache. Vous obtenez ainsi une bien meilleure performance qu’avec les API XAML, car l’image n’a pas besoin d’être ouverte ou décodée.
