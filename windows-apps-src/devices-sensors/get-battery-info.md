...
Découvrez comment obtenir des informations détaillées sur la batterie à l’aide des API de l’espace de noms [**Windows.Devices.Power**](/uwp/api/Windows.Devices.Power). Un *rapport sur la batterie*([**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport)) décrit la charge, la capacité et l’état d’une batterie ou d’un groupe de batteries. Cette rubrique montre comment votre application peut obtenir des rapports sur la batterie et être avertie des modifications. Les exemples de code sont tirés de l’application de batterie de base mentionnée à la fin de cette rubrique.

## <a name="get-aggregate-battery-report"></a>Obtenir un rapport sur un groupe de batteries


Certains appareils utilisent plusieurs batteries et il n’est pas toujours évident de déterminer de quelle manière chacune d’elles contribue à la capacité globale d’alimentation de l’appareil. C’est là que la classe [**AggregateBattery**](/uwp/api/windows.devices.power.battery.aggregatebattery) entre en jeu. Le *groupe de batteries* représente tous les contrôleurs de batterie connectés à un appareil et peut fournir un objet [**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport) global.

**Remarque**    Une classe de [**batterie**](/uwp/api/Windows.Devices.Power.Battery) correspond en fait à un contrôleur de batterie. Le contrôleur est tantôt relié à la batterie physique de l’appareil, tantôt à son boîtier. Par conséquent, il est possible de créer un objet batterie, même en l’absence de toute batterie. Dans certains cas, l’objet batterie peut être **null**.

Une fois que vous avez un objet de batterie Aggregate, appelez [**GetReport**](/uwp/api/windows.devices.power.battery.getreport) pour obtenir le [**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport)correspondant.
...
Vous pouvez également créer un objet [**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport) pour des batteries individuelles. Utilisez [**GetDeviceSelector**](/uwp/api/windows.devices.power.battery.getdeviceselector) avec la méthode [**FindAllAsync**](/uwp/api/windows.devices.enumeration.deviceinformation.findallasync) pour obtenir une collection d’objets [**DeviceInformation**](/uwp/api/Windows.Devices.Enumeration.DeviceInformation) qui représentent tous les contrôleurs de batterie connectés à l’appareil. Ensuite, à l’aide de la propriété **Id** de l’objet **DeviceInformation** souhaité, créez une classe [**Battery**](/uwp/api/Windows.Devices.Power.Battery) correspondante avec la méthode [**FromIdAsync**](/uwp/api/windows.devices.power.battery.fromidasync). Enfin, appelez la méthode [**GetReport**](/uwp/api/windows.devices.power.battery.getreport) pour obtenir le rapport sur la batterie individuelle.
...
L’objet [**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport) fournit un grand nombre d’informations sur la batterie. Pour plus d’informations, reportez-vous à la référence de l’API pour connaître ses propriétés : **Status** (énumération [**BatteryStatus**](/previous-versions/windows/dn818458(v=win.10))), [**ChargeRateInMilliwatts**](/uwp/api/windows.devices.power.batteryreport.chargerateinmilliwatts), [**DesignCapacityInMilliwattHours**](/uwp/api/windows.devices.power.batteryreport.designcapacityinmilliwatthours), [**FullChargeCapacityInMilliwattHours**](/uwp/api/windows.devices.power.batteryreport.fullchargecapacityinmilliwatthours) et [**RemainingCapacityInMilliwattHours**](/uwp/api/windows.devices.power.batteryreport.remainingcapacityinmilliwatthours). Cet exemple présente certaines des propriétés du rapport sur la batterie utilisées par l’application de batterie de base fournie plus loin dans cette rubrique.
...
L’objet [**Battery**](/uwp/api/Windows.Devices.Power.Battery) déclenche l’événement [**ReportUpdated**](/uwp/api/windows.devices.power.battery.reportupdated) lorsque la charge, la capacité ou l’état de la batterie change. Cela se produit généralement immédiatement pour les changements d’état, et périodiquement pour tous les autres changements. Cet exemple montre comment s’inscrire pour les mises à jour de rapport sur la batterie.
...
En cas de mise à jour de la batterie, l’événement [**ReportUpdated**](/uwp/api/windows.devices.power.battery.reportupdated) transmet l’objet [**Battery**](/uwp/api/Windows.Devices.Power.Battery) correspondant à la méthode du gestionnaire d’événements. Cependant, ce gestionnaire d’événements n’est pas appelé à partir du thread d’interface utilisateur. Vous devez utiliser l’objet [**répartiteur**](/uwp/api/Windows.UI.Core.CoreDispatcher) pour appeler toute modification de l’interface utilisateur, comme illustré dans cet exemple.
...
**Conseil**    Pour recevoir des valeurs numériques de l’objet [**BatteryReport**](/uwp/api/Windows.Devices.Power.BatteryReport) , déboguez votre application sur l' **ordinateur local** ou sur un **périphérique** externe (tel qu’un Windows Phone). Lors du débogage sur un émulateur d’appareil, l’objet **BatteryReport** renvoie **null** aux propriétés de capacité et de vitesse.
