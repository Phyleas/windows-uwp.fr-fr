...
Transmettez les URI absolus, et non relatifs, aux types Windows Runtime. Voir [Transmission d’un URI au Windows Runtime](/dotnet/standard/cross-platform/passing-a-uri-to-the-windows-runtime).
...
Port auquel le code appelle la méthode [**REQUESTAPPPURCHASEASYNC**](/uwp/api/windows.applicationmodel.store.currentapp.requestapppurchaseasync) UWP :
...
Si vous avez du code qui simule votre achat d’application et les fonctionnalités d’achat dans l’application à des fins de test, vous pouvez alors le porter pour utiliser la classe [**CurrentAppSimulator**](/uwp/api/Windows.ApplicationModel.Store.CurrentAppSimulator) à la place.
...
Pour plus d’informations sur l’utilisation des notifications dans une application UWP, voir [Envoi de notifications toast](/previous-versions/windows/apps/hh868266(v=win.10)).
...
Une application Silverlight pour Windows Phone dispose d’un accès en lecture seule à la carte mémoire Secure Digital en option. Une application UWP présente quant à elle un accès en lecture et en écriture à la carte mémoire Secure Digital. Pour plus d’informations, voir [Accéder à la carte SD](../files/access-the-sd-card.md).

Pour plus d’informations sur l’accès aux fichiers photo, musique et vidéo dans une application UWP, voir [Fichiers et dossiers dans les bibliothèques de musique, d’images et de vidéos](../files/quickstart-managing-folders-in-the-music-pictures-and-videos-libraries.md).

Pour plus d’informations, voir [Fichiers, dossiers et bibliothèques](../files/index.md).
